...
const BusinessRealEstateDiscountsPage = () => {
  ...
  const loadData = () => {
    ...
    setDiscounts(partnerDiscounts.filter((d) => d.scope === 'REAL_ESTATE'));
  };

  const filteredDiscounts = useMemo(() => {
    return discounts.filter((discount) => {
      if (filters.status === 'ACTIVE' && !discount.isActive) return false;
      if (filters.status === 'INACTIVE' && discount.isActive) return false;
            ...
            >
              <div className="flex items-start justify-between mb-4">
                <div className="flex-1">
                  ...
                  <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-indigo-100 text-indigo-800 text-sm font-semibold">
...
export default BusinessRealEstateDiscountsPage;
